...
Name | Install Steps | Description / Config
---- |------------- | --------------------
<img src=images/rustacean.svg height=18/> [deff](https://github.com/flamestro/deff) | `cargo install deff` | TUI alternative to "git diff". Arg structure is different for specifying what to diff, but not too bad, and the TUI interface is quite nice.
...
<img src=images/rustacean.svg height=18/> [sem](https://github.com/Ataraxy-Labs/sem) | Download binary. Place in *~/.local/bin* | Gives semantic understanding of what is changing in diffs. Also of potential impact of changes across code base.
...
<img src=images/gopher.svg height=18> [fzf](https://github.com/junegunn/fzf) | `apt install fzf`, then look at `apt show fzf` to find instructions on enabling keybindings and autocompletions. | Powerful "command-line fuzzy finder" that is hard to describe adequately. Its *Alt-C* function has allowed me to augment *autojump* when jumping *into* the current tree. Just watch [this YouTube video](https://youtu.be/qgG5Jhi_Els), or read [this blog post](https://andrew-quinn.me/fzf/).
...
<img src=images/python.svg height=18/> [litecli](https://github.com/dbcli/litecli) | `pip install litecli` | Colorful autocomplete-savvy SQLite CLI client
...
<img src=images/rustacean.svg height=18/> [zellij](https://github.com/zellij-org/zellij) | `snap install --classic zellij`  | Colorful, user-friendly, `tmux` replacement, that I usually use now.  **Config**: Copy *.config/zellij/config.yaml* to use Alt-S and Alt-Q instead, which avoids collision with *micro* editor keys.
...
**dpkg** = Find the appropriate [non-musl-variant](https://www.musl-libc.org/faq.html)
...
It is not supported on WSL. **flatpak** must first be
[installed and setup](https://flathub.org/en/setup/Ubuntu).
